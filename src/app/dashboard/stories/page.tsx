...
export default function StoriesPage() {
  ...
  // Stories nach Team gruppieren für die Story-Ringe oben
  const teamMap = new Map<string, { team: StoryTeam; count: number }>();
  for (const story of stories) {
    if (story.team) {
      ...
      if (existing) {
        ...
        teamMap.set(story.team.id, { team: story.team, count: 1 });
      }
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700 sticky top-0 z-40">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <Link href="/dashboard/stories/new" className="text-sm text-blue-600 dark:text-blue-400 font-medium">
            + Posten
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto">
        {/* Team-Ringe (wie Instagram Story Circles) */}
          ...
          <div className="flex gap-4 overflow-x-auto px-4 py-4 border-b dark:border-gray-800">
            {activeTeams.map(({ team, count }) => (
              <div key={team.id} className="flex flex-col items-center gap-1 flex-shrink-0">
                <div className="p-[3px] rounded-full" style={{ background: `linear-gradient(135deg, ${team.color}, ${team.color}88)` }}>
                  <div className="w-14 h-14 rounded-full bg-white dark:bg-gray-900 flex items-center justify-center p-[2px]">
                    <div className="w-full h-full rounded-full flex items-center justify-center text-white font-bold text-lg" style={{ backgroundColor: team.color }}>
                      {team.name.charAt(0)}
                    </div>
                  </div>
                </div>
                <span className="text-[10px] text-gray-500 dark:text-gray-400 max-w-[64px] truncate text-center">
                  {team.name}
                </span>
        ...
        <div className="divide-y dark:divide-gray-800">
          {loading ? (
            ...
            stories.map((story) => (
              <div key={story.id} className="bg-white dark:bg-gray-800">
                {/* Story Header */}
                ...
                {story.text && (
                  <div className={`px-4 ${story.mediaUrl ? "pt-3" : ""} ${!story.mediaUrl ? "py-4" : ""}`}>
                    <p className={`text-gray-900 dark:text-white leading-relaxed ${!story.mediaUrl ? "text-lg" : "text-sm"}`}>
                      {story.text}
                    </p>
                  </div>
                )}
                ...
                <div className="px-4 py-2">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {Object.entries(story.emojiGroups).map(([emoji, group]) => (
                      ...
                      >
                        <span>{emoji}</span>
                        <span className={`font-medium ${group.hasMyReaction ? "text-blue-700 dark:text-blue-300" : "text-gray-600 dark:text-gray-400"}`}>
                          {group.count}
                        </span>
                      </button>
                    ))}
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 text-sm transition-colors"
                      >
                        +
                      </button>
                      {showEmojiPicker === story.id && (
                        <div className="absolute bottom-full left-0 mb-1 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl shadow-xl p-2 flex gap-1 z-10">
                ...
                {expandedStory === story.id && (
                  <div className="px-4 pb-4 border-t dark:border-gray-700 pt-3">
                    ...
                    <div className="flex gap-2">
                      <input
                        type="text"
                        value={expandedStory === story.id ? newComment : ""}
                        onChange={(e) => setNewComment(e.target.value)}
                        placeholder="Kommentar schreiben..."
                        maxLength={500}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") handleComment(story.id);
                        }}
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-blue-600 text-white rounded-full text-xs font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors"
                      >
                        Senden
                      </button>
                    </div>
