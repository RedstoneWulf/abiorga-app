...
export default function NewStoryPage() {
  ...
  const fetchTeams = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        const memberTeams = data.filter((t: Team) => t.isMember);
        setTeams(memberTeams);
        if (preselectedTeamId && memberTeams.some((t: Team) => t.id === preselectedTeamId)) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!selectedTeam) { setError("Wähle ein Team"); return; }
    if (!text.trim() && !imagePreview) { setError("Text oder Bild erforderlich"); return; }
    ...
    try {
      ...
      if (res.ok) router.push("/dashboard/stories");
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-3 mb-5">
          <p className="text-xs text-blue-700 dark:text-blue-300">Stories sind 48 Stunden sichtbar und verschwinden dann automatisch.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Team */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Posten als Team *</label>
            {teams.length === 0 ? (
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {teams.map((team) => (
                  <button key={team.id} type="button" onClick={() => setSelectedTeam(team.id)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border-2 flex-shrink-0 transition-all ${
                      selectedTeam === team.id ? "border-blue-500 bg-blue-50 dark:bg-blue-900/30" : "border-gray-200 dark:border-gray-600 hover:border-gray-300"
                    }`}>
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold"
                      style={{ backgroundColor: team.color }}>{getTeamIcon(team)}</div>
          ...
          {/* Bild */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Bild (optional)</label>
          ...
          {/* Text */}
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Text {imagePreview ? "(optional)" : "*"}</label>
            <textarea value={text} onChange={(e) => setText(e.target.value)}
              placeholder="Was gibt's Neues? 🎉" rows={imagePreview ? 2 : 4} maxLength={1000}
              ...
              <div className="bg-white dark:bg-gray-800 p-3">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold"
                    style={{ backgroundColor: selectedTeamObj.color }}>{getTeamIcon(selectedTeamObj)}</div>
                ...
                {imagePreview && <img src={imagePreview} alt="" className="w-full max-h-[200px] object-contain rounded-lg mb-2 bg-gray-100 dark:bg-gray-900" />}
                {text.trim() && <p className="text-sm text-gray-900 dark:text-white">{text}</p>}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading || !selectedTeam || (!text.trim() && !imagePreview)}
              ...
              {loading ? "Wird gepostet..." : "Story posten"}</button>
          </div>
        </form>
      </main>
    </div>
