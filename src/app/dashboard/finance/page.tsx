...
export default function FinancePage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setShowCreate(false);
        setNewTx({ amount: "", type: "INCOME", reason: "", category: "" });
  ...
  async function handleGoalUpdate() {
    ...
    try {
      const res = await fetch("/api/finance", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ financeGoal: goalValue }),
      });
  ...
  const goalPercent = Math.min(100, Math.round((overview.balance / overview.goal) * 100));
      ...
      <main className="max-w-4xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 p-5">
          ...
          {showGoalEdit && (
            <div className="flex gap-2 mb-3">
              ...
              <button type="button" onClick={handleGoalUpdate}
        ...
        <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
          {[
            ...
            { key: "transactions" as const, label: "Alle Transaktionen" },
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${STATUS_COLORS[tx.status]}`}>
        ...
        {/* --- Tab: Genehmigungen (nur Admin/Committee) --- */}
                  ...
                  {rejectId === tx.id ? (
                    ...
                    <div className="flex gap-2">
                      ...
                      <button type="button" onClick={() => setRejectId(tx.id)}
                        className="flex-1 py-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg text-xs font-medium hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors">
                        Ablehnen
                      </button>
                    </div>
                  )}
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setNewTx({ ...newTx, type: "INCOME" })}
                  className={`p-3 rounded-xl border-2 text-center transition-all ${
                    newTx.type === "INCOME" ? "border-green-500 bg-green-50 dark:bg-green-900/30" : "border-gray-200 dark:border-gray-600"
                  }`}>
                ...
                <button type="button" onClick={() => setNewTx({ ...newTx, type: "EXPENSE" })}
                  className={`p-3 rounded-xl border-2 text-center transition-all ${
                    newTx.type === "EXPENSE" ? "border-red-500 bg-red-50 dark:bg-red-900/30" : "border-gray-200 dark:border-gray-600"
                  }`}>
                  <div className="text-lg mb-0.5">🧾</div>
              ...
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                <p className="text-xs text-yellow-700 dark:text-yellow-400">
                  Transaktionen müssen vom Finanz-Team oder Komitee genehmigt werden bevor sie im Kontostand erscheinen.
                </p>
              </div>

              <button type="submit" disabled={createLoading}
                className="w-full py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors">
                {createLoading ? "Wird erstellt..." : "Transaktion einreichen"}
              </button>
            </form>
          </div>
        </div>
